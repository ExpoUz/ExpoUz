  ...
  return (
    <div className="min-h-screen pb-12 px-4 pt-6">
      ...
      <div className="flex flex-col items-center text-center">
        ...
        {me && me.id !== id && (
          ...
          >
            ...
            {opening ? t("opening") : t("message")}
          </button>
        )}
      </div>

      {/* Football stats */}
      <div className="text-xs font-semibold mt-5 mb-2 px-1" style={{ color: "var(--tg-hint)" }}>{t("football")}</div>
      <div className="grid grid-cols-4 gap-2">
        <Stat label={t("games")} value={p.gamesAttended ?? 0} />
        <Stat label={t("thisMonth")} value={p.gamesThisMonth ?? 0} />
        <Stat label={t("elo")} value={p.eloRating ?? 1000} />
        <Stat label={t("ratedUp")} value={p.ratings?.thumbsUp ?? 0} />
      </div>
      ...
      {(() => {
        ...
        return (
          <>
            <div className="text-xs font-semibold mt-4 mb-2 px-1" style={{ color: "var(--tg-hint)" }}>{t("padel")}</div>
            {assessed ? (
              <div className="rounded-2xl p-4 flex items-center gap-4" style={{ background: "var(--tg-card)" }}>
                <div className="text-center">
                  ...
                  <div className="text-[11px] font-semibold" style={{ color: band.color }}>{tBands(band.key)}</div>
                </div>
                <div className="flex-1 grid grid-cols-3 gap-2 text-center">
                  <Stat label={t("played")} value={p.padelMatchesPlayed ?? 0} />
                  <Stat label={t("won")} value={p.padelMatchesWon ?? 0} />
                  <Stat label={t("reliable")} value={`${p.padelReliability ?? 0}%`} />
                </div>
              </div>
            ) : (
      ...
      {ranking && ranking.levelInfo?.nextAt != null && (
        <div className="rounded-2xl p-4 mt-4" style={{ background: "var(--tg-card)" }}>
          <div className="flex items-center justify-between text-sm mb-2">
            <span className="font-semibold">{t("progressToNext")}</span>
            <span style={{ color: "var(--tg-hint)" }}>{t("gamesToNext", { count: ranking.gamesToNext })}</span>
          </div>
      ...
      {(p.recentMatches ?? []).length > 0 && (
        <div className="mt-5">
          <div className="text-sm font-semibold mb-2 px-1">{t("recentGames")}</div>
            ...
            {p.recentMatches.map((b: any) => (
              <div key={b.id ?? b.match?.id} className="rounded-2xl p-3 flex items-center justify-between" style={{ background: "var(--tg-card)" }}>
                <div className="min-w-0">
                  <div className="text-sm font-medium truncate">{b.match?.title ?? t("matchFallback")}</div>
